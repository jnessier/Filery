...
class ApiClient_ApiClient {
    ...
    static async fetchFiles(dir) {
        return await client["get"](this.baseUrl)
            .query({
            'action': 'fetch',
            'dir': dir,
        })
    ...
    static async uploadFile(fileData, dir) {
        return await client('POST', this.baseUrl)
            .withCredentials()
            .query({
            'action': 'upload',
            'dir': dir,
    ...
    static async deleteFile(file, dir) {
        return await client('GET', this.baseUrl)
            .withCredentials()
            .query({
            'action': 'delete',
...
class Control {
    ...
    static select(query) {
        let element = document.querySelector(query), control = new this();
        console.log(element);
        if (element) {
    ...
    static create(tagName, attributes) {
        ...
        let element = document.createElement(tagName), control = new this();
...
// CONCATENATED MODULE: ./src/ts/Filery/UI/FileThumbnailImage.ts

class FileThumbnailImage_FileThumbnailImage extends Control {
    constructor(file) {
        super();
...
class FileThumbnail_FileThumbnail extends Control {
    constructor(file) {
        ...
        this
            .append(new FileThumbnailImage_FileThumbnailImage(file));
    }
}

// CONCATENATED MODULE: ./src/ts/Filery/UI/FileTitleName.ts

class FileTitleName_FileTitleName extends Control {
    constructor(file) {
        super();
        this.element = document.createElement('span');
...
// CONCATENATED MODULE: ./src/ts/Filery/UI/FileTitleMetaItem.ts

class FileTitleMetaItem_FileTitleMetaItem extends Control {
    constructor(value) {
        super();
...
// CONCATENATED MODULE: ./src/ts/Filery/UI/FileTitleMeta.ts


class FileTitleMeta_FileTitleMeta extends Control {
    constructor(file) {
        super();
        this.element = document.createElement('ul');
        this.element.classList.add('file-title-meta');
        this.append(new FileTitleMetaItem_FileTitleMetaItem(file.getFormattedSize()));
        this.append(new FileTitleMetaItem_FileTitleMetaItem(file.getFormattedTime()));
    }
}
...
class FileTitle_FileTitle extends Control {
    constructor(file) {
        ...
        this
            .append(new FileTitleName_FileTitleName(file))
            .append(new FileTitleMeta_FileTitleMeta(file));
    }
}

// CONCATENATED MODULE: ./src/ts/Filery/UI/ItemBody.ts



class ItemBody_ItemBody extends Control {
    constructor(file) {
        super();
        this.element = document.createElement('div');
...
class Item_Item extends Control {
    constructor(file) {
        ...
        this.append(new ItemBody_ItemBody(file));
    }
    toggleSelect() {
...
class Container_Container extends Control {
    constructor(files) {
        ...
        files.forEach((file) => {
            this.append(new Item_Item(file));
        });
    }
}
...
class Plugin_Plugin {
    ...
    loadDialogContent() {
        ApiClient_ApiClient
            ...
            .then((result) => {
            if (result.status) {
                let container = new Container_Container(result.data), dialogBody = Control.select('#filery-dialog-body');
...
/* harmony default export */ var ts_plugin = (function (editor, url) {
    ...
    ApiClient_ApiClient.setBaseUrl(editor.settings.filery.api.base_url);
