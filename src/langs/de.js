tinymce.addI18n('de', {
    // Buttons
    'Delete': 'Löschen',
    'Select': 'Auswählen',
    'Upload': 'Hochladen',
    'Insert link': 'Link einfügen',
    'Insert image': 'Bild einfügen',
    'Cancel': 'Abbrechen',

    // Dialog
    'File manager': 'Dateiverwaltung',


    'Are you sure you want to delete "{0}"?': 'Bist du sicher, dass du "{0}" löschen willst?',

    'Upload failed.': 'Hochladen fehlgeschlagen.',
    'Delete failed.': 'Löschen fehlgeschlagen.',

    '"{0}" successfully uploaded.': '"{0}" erfolgreich hochgeladen.',
    '"{0}" successfully deleted. Do you want to remove the content with reference to the deleted file?': '"{0}" erfolgreich gelöscht. Willst du den Inhalt mit Bezug zur gelöschen Datei entfernen?',
    '"{0}" successfully inserted.': '"{0}" erfolgreich eingefügt.',

    // Notifications
    '"{0}" as image successfully inserted.': '"{0}" als Bild erfolgreich eingefügt.',
    '"{0}" as link successfully inserted.': '"{0}" als Link erfolgreich eingefügt.',

    // asd

    'File already exists.': 'Die Datei existiert bereits.',
    'File is too large.': 'Die Datei ist zu gross.',
    'File extension is not allowed.': 'Die Dateierweiterung ist nicht erlaubt.'

});